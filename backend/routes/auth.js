...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    // res.json({success:false, message: 'Be patient your account verification still in process' });
...
router.get("/getStudents", async (req, res) => {
  try {
    const Total_Users = await User.find({ role: "user"}).select("-password");
    const Total_approved_Users = await User.find({ role: "user" ,approved:true}).select("username email");
    const Total_pending_Users = await User.find({ role: "user" ,approved:false}).select("username email");
